...
import { fileList } from '../React/Plugin/Upload/SummaryUpload/api';
...
export function getSummaryUploadInformationsFiles(
  currentFilesList,
  managementCloudId,
  token,
  languageCode
) {
  const formatedData = currentFilesList.map((file) => ({
    Id: file?.Models?.[0]?.ModelId,
    MediaType: file?.Models?.[0]?.MediaType,
  }));

  fileList(languageCode, managementCloudId, formatedData).then((json) => {
    const action = { data: json, type: 'setSummaryUploadDetailsListFiles' };
    store.dispatch(action);
    return json;
  });
}
