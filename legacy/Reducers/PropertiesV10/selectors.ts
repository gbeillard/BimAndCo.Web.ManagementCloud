...
import { DATA_TYPES } from '../../React/Properties/PropertyPicker/constants';
import { baseState } from './reducer';
import { Property, FilterSort, Domain } from './types';
import { searchProperties, sortObjectArray, filterByDomain, extractDomains, filterPropertiesByDataTypes } from './utils';

const selectRoot = (store): typeof baseState => store.propertiesV10State;
...
export const selectUpdatePropertySubsetsIsError = createSelector(
  selectRoot,
  (state): string => state.api.updatePropertySubsets.error
);
...
export const selectClassificationsProperties = createSelector(
  selectProperties,
  selectFilterSort,
  (properties, sort): Property[] => {
    const dataTypes = [DATA_TYPES.STRING, DATA_TYPES.TEXT];
    const filtered = filterPropertiesByDataTypes(properties, dataTypes);
    return filteredProperties(filtered, sort);
  } 
);
