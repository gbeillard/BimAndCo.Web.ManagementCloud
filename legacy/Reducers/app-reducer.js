  ...
  switch (action.type) {
    ...
    case types.UPDATE_INFORMATIONS_CONTEXT:
      if (action.data.EntityId != undefined) {
        ...
        console.log('UPDATE_INFORMATIONS_CONTEXT', action.data);
        return {
          ...state,
    ...
    case 'setBundles': {
      ...
      if (action.data != null && action.data.BundleList != null) {
        ...
        if (!state.Settings?.EnableDictionary) {
          let xmlDocs = [];
          for (var i = 0; i < action.data.BundleList.length; i++) {
    ...
    case 'setSummaryUploadListFiles':
      if (action?.data) {
        // fileList informations load
        SearchApi.getSummaryUploadInformationsFiles(
          action.data,
          state.ManagementCloudId,
          state.TemporaryToken,
          state.Language
        ...
        // store ids for later calls
        return { ...state, SummaryUploadListFiles: action.data };
      }
      return state;
    ...
    case 'setSummaryUploadDetailsListFiles':
      if (action?.data) {
        return { ...state, SummaryUploadDetailsListFiles: action.data };
      }
      return state;
    ...
    case GET_USER_INFO_SUCCESS:
      ...
      OCAnalytics.config({
        role: action.userInfo?.Role?.Key,
        dataLanguage: action.userInfo.DataLanguage,
      });
    ...
    case SET_DATA_LANGUAGE:
      return { ...state, DataLanguage: action.dataLanguage };
    default:
      return state;
  }
  return state;
