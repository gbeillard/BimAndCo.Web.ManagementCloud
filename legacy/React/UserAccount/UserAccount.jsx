...
import { SET_DATA_LANGUAGE } from '../../Reducers/app/constants';
...
let UserAccount = createReactClass({
    ...
    if (userDetails.FirstName == '') {
      ...
      fetch(`${API_URL}/api/ws/v1/user/account/update?token=${self.props.TemporaryToken}`, {
        ...
        body: JSON.stringify({
          ...
          DataLanguage: userDetails.DataLanguage,
        ...
        .then((response) => {
          if (response.status == 200) {
            store.dispatch({ type: SET_DATA_LANGUAGE, dataLanguage: userDetails.DataLanguage });
  ...
  handleChangeDataLanguage(event) {
    this.state.UserDetails.DataLanguage = event.target.value;
    this.forceUpdate();
  },
  ...
  render() {
    ...
    if (user != null) {
      ...
      let dataLangValue = user.DataLanguage ? user.DataLanguage : null;

      const defaultLanguageSelectList = [];
      const dataLanguageSelectList = this.props.cultures?.map((culture) => 
        <MenuItem key={culture?.Code} value={culture?.Code} data-cy={culture?.Name}>
          <a>{culture?.Name}</a>
        </MenuItem>
      );
      ...
      _.each(self.props.Languages, (lang) => {
        ...
        const languageLine = (
          <a className={`change-lang-dropdown-link lang-${lang.LanguageCode}`}> {language_trad} </a>
        );
        ...
        if (user.DataLanguage === lang.LanguageCode) {
          dataLangValue = {
            value: dataLangValue,
            label: lang.Translations[self.props.Language],
          };

          if(!self.props.cultures.some(culture => culture?.Code === lang.LanguageCode)){
            dataLanguageSelectList.unshift(
              <MenuItem key={lang.LanguageCode} value={lang.LanguageCode} data-cy={language_trad}>
                <a>{language_trad}</a>
              </MenuItem>
            );
        ...
        <div className="edit-profile-panel">
          ...
          <div className="row up-account-part">
            ...
            <div className="col-xs-10 col-md-4 col-xs-offset-1">
              ...
              <div className="row">
                <FormControl className="formControl_userAccount">
                  <InputLabel shrink htmlFor="userDataLang-label">
                    {self.props.resources.UserProfileEdition.DataLanguagePlaceHolder}
                  </InputLabel>
                  <Select
                    data-cy={self.props.resources.UserProfileEdition.DataLanguagePlaceHolder}
                    required
                    displayEmpty
                    id="userDataLang"
                    error={user.DataLanguage === ''}
                    value={dataLangValue.value}
                    onChange={self.handleChangeDataLanguage}
                    style={{ width: '100%' }}
                  >
                    {dataLanguageSelectList}
                  </Select>
                </FormControl>
              </div>
            </div>
...
export default UserAccount = connect(mapStateToProps)(withCountriesCultures(UserAccount));
