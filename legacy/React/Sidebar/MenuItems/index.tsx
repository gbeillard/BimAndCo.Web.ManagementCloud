...
const ChildrenList = styled.ul(({ maxHeight }: { maxHeight: string }) => ({
  position: 'absolute',
  left: '50px',
  width: '330px',
  overflowY: 'auto',
...
const MenuItem: React.FC<Props> = ({
  ...
  const checkBoundaries = () => {
    if (subMenuRef.current) {
      ...
      if (spaceBottom < 0) {
        setMaxHeight(`${rect.height + spaceBottom}px`);
      } else if (spaceTop < 0) {
        setMaxHeight(`${rect.height + spaceTop}px`);
      } else {
        setMaxHeight('none');
      }
    }
  ...
  return (
    <ListItem>
      ...
      {children && (
        <ChildrenList className="sub-menu" ref={subMenuRef} maxHeight={maxHeight}>
          {children}
        </ChildrenList>
      )}
    </ListItem>
