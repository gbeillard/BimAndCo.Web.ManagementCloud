...
import { ModalEditTableProperty } from './ModalEditTableProperty';
...
let EditorStepProperties = createReactClass({
  getInitialState() {
    ...
    return {
      ...
      initialData: [],
      isOpenModal: false,
      instCoordTable: { r: -1, c: -1 },
      dataTable: '',
      propertySchemaTable: null,
    };
  },
  ...
  addSettings(hot) {
    ...
    if (self.state.saveEnabled) {
      ...
      hot.addHook('afterOnCellMouseDown', (event, coords) => {
        const property = self.state.data.Properties.filter(
          (e) => e.Name === hot.getColHeader()[coords.col]
        )[0];
        if (property?.IsTable) {
          self.setState({
            isOpenModal: true,
            instCoordTable: { r: coords.row, c: coords.col },
            dataTable: hot.getSourceDataAtCell(coords.row, coords.col),
            propertySchemaTable: property,
          });
        }
      });
      ...
      if (!this.state.validDatas && errors.length > 0) {
        ...
        _.each(errors, (domainError) => {
          ...
          _.each(errorsByProperty, (error) => {
            ...
            const dataLanguage = _.contains(error.langs, self.props.Language)
              ? self.props.Language
              : error.langs[0];

            const errorElement = $(`
              <li>
                ${self.props.resources.EditPropertiesPage.PropertyLabel} 
                : ${error.property.propertyName} 
                - ${self.props.resources.EditPropertiesPage.RowLabel} 
                : ${error.rows.join('/')} 
                - ${self.props.resources.EditPropertiesPage.ValueLabel} 
                : ${error.property.value} (${error.property.type}) 
                - 
                <a 
                  data-language='${dataLanguage}'
                  data-domain-id='${domainError.domainId}'
                  data-row='${parseInt(error.rows[0]) - 1}'
                  data-property-id='${error.property.propertyId}'
          ...
          $('.see-error').on('click', function () {
            const currentDomainId = $(this).data('domain-id');
            ...
            hot.selectCell(parseInt(hot.propToCol(currentRow)), hot.propToCol(currentPropertyId));
          });
        });
  ...
  selectDomain(event) {
    ...
    const currentLanguage = this.state.currentLang ?? this.state.currentLanguage;
  ...
  render() {
    ...
    const getInitDataTable = (str) => {
      if (str === '' || str === []) return { Row: [] };
      return JSON.parse(str);
    };

    return (
      <div>
        <ModalEditTableProperty
          className="modal-edit-table-property"
          resources={this.props.resources}
          initData={getInitDataTable(self.state.dataTable)}
          isOpen={self.state.isOpenModal}
          sendData={(data) => {
            self.setState({ isOpenModal: false });
            if (data !== '') {
              const currentHot = handsontableEditor.findHandsontableInstance(
                handsontableEditor.container,
                handsontableEditor.handsontableInstances
              );
              currentHot.setDataAtCell(
                self.state.instCoordTable.r,
                self.state.instCoordTable.c,
                data
              );
            }
            self.setState({ isOpenModal: false, instCoordTable: { r: -1, c: -1 }, dataTable: '' });
          }}
          property={self.state.propertySchemaTable}
        />
              ...
              <tr id={property.Id} className={className} key={property.Id}>
                <td>
                  <span className="order" data-attribute="Order" data-id={property.Id}>
                    {properties.every(p => p.Order === 0) ? propertyIndex + 1 : property.Order}
                  </span>
                </td>
                <td>{property.Name}</td>
                <td>{options}</td>
