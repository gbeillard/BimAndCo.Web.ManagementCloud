...
const mapOptionsToVariants = (options: Option[]): ObjectVariant[] =>
  options?.map((option) => ({
    Id: option?.value,
    Name: option?.label,
  }));
...
const ImportForm: React.FC<Props> = ({
  ...
  const onVariantsChange = (variants: Option[]) => {
    onDocumentChange({ ...document, Variants: mapOptionsToVariants(variants) });
  };
