...
import { withCountriesCultures } from '@bim-co/onfly-connect/OCProviders';
import { resolveDistributionCountries } from './utils'
...
let EditorInformation = createReactClass({
    ...
    return {
      objectType:
        this.props.initialData != null
          ...
          : objectTypeRef[0],
      isChanged: false,
      isTemplate: this.props.initialData?.IsTemplate ?? false,
      newObjectType: null,
      selectedManuf: 0,
      selectedCompany: 0,
      loading: false,
  ...
  componentWillReceiveProps(nextProps) {
    if (
      ...
      this.setState({
        isTemplate: nextProps.initialData.IsTemplate,
      });
  ...
  _handleChangeIsTemplate(value) {
    if (this.props.bimObjectId) {
      this.props.setIsTemplate(value);
      this._updateBimObjectIsTemplate(value);
    } else {
      this.setState({ isTemplate: value });
    }
  },

  _updateBimObjectIsTemplate(value) {
    fetch(
      `${API_URL}/api/v10/onfly/${this.props.managementCloudId}/bimobjects/${this.props.bimObjectId}`,
      {
        method: 'PATCH',
        headers: {
  ...
  render() {
    ...
    if (
      ...
      const countries = resolveDistributionCountries({ 
    ...
    return (
      <div>
        <ObjectUpgrade
          ...
          EnableObjectTemplate={this.props.Settings.EnableObjectTemplate}
          ...
          _handleChangeIsTemplate={this._handleChangeIsTemplate}
...
export default connect(mapStateToProps)(withCountriesCultures(EditorInformation));
