...
import { withCountriesCultures } from '@bim-co/onfly-connect';
import { resolveModelLanguages } from './utils';
...
let EditorStepModels = createReactClass({
  ...
  validateUpload() {
    ...
    if ($('#choice-variants-modal input:checkbox:checked').length > 0) {
      ...
      toastr.error(self.props.resources.ContentManagement.SelectAtLeastOneVariant);
    }
  },
  ...
  deleteModel() {
    ...
    if ($('#confirm-deletion-modal input:checkbox:checked').length > 0) {
      $('#confirm-deletion-modal input:checkbox:checked').each((index, element) => {
  ...
  changeSoftware(event) {
    ...
    this.editModel(modelId, modelType, value, null, null, null, null, null);
  },
  ...
  changeLod(event) {
    ...
    this.editModel(modelId, modelType, null, value, null, null, null, null);
  },
  ...
  changeCountry(event) {
    ...
    this.editModel(modelId, modelType, null,  null, value || 0, null, null, null);
  },
  ...
  changeLanguage(event) {
    ...
    this.editModel(modelId, modelType, null, null, null, value || "", null, null);
  },
  ...
  setValue(selectedSubset, currentSubsets) {
    ...
    const currentModel =
      this.state.data[this.state.selectedVariantId].Models[this.state.selectedModelId];
  ...
  confirmChangeName(event) {
    ...
    const modelType = event.target.dataset.type;
    const value = $(
      `#v${this.state.selectedVariantId}m${modelId}t${modelType.substring(0, 2)}-name`
    ).val();

    this.editModel(modelId, modelType, null, null, null, null, value, null);
  ...
  editModel(modelId, modelType, softwareId, Lod, countryId, languageCode, Name, typeFormat) {
    const self = this;
    let url = '';
  ...
  handleClickOnCheckbox(event) {
    const ancestor =
      event.target.parentNode.parentNode.parentNode.parentNode.parentNode.parentNode.parentNode;

    if (event.target.value === 'allVariantsE') {
      if (event.target.checked) {
        $('#confirm-deletion-modal input[name="variants"]').prop('checked', true);
      } else {
        $('#confirm-deletion-modal input[name="variants"]').prop('checked', false);
      }
    } else if (event.target.value === 'allVariantsU') {
      if (event.target.checked) {
        $('#choice-variants-modal input[name="variants"]').prop('checked', true);
      } else {
        $('#choice-variants-modal input[name="variants"]').prop('checked', false);
      }
  ...
  fetchExtensions() {
    const self = this;
    const url = `${API_URL}/api/v10/onfly/${this.props.managementCloudId}/extensionsfile`;
    ...
    fetch(url, {
      ...
      headers: {
        Accept: 'application/json',
        'Content-Type': 'application/json',
        Authorization: `Bearer ${this.props.TemporaryToken}`,
        'Api-Key': localStorage.getItem('ApiKey')|| sessionStorage.getItem('ApiKey') || '',

      },
    })
      .then((response) => response.json())
      .then((json) => {
        self.setState({ extensions: json });
      });
  },
  ...
  render() {
    ...
    if (this.state.data.length !== 0) {
      ...
      rowsVariants = _.map(this.state.data, (variant, i) => {
        ...
        return (
          <tr key={i} data-id={i} onClick={() => self.selectVariant(i)} className={classStr}>
            <td>{cpt}</td>
            <td>{variant.Ref}</td>
      ...
      variantsSelection = _.map(this.state.data, (variant, i) => {
        if (self.state.deletedModelId === 0 || variant.Models[self.state.deletedModelId] != null) {
          return (
            <div key={i}>
              <label>
                <input
    ...
    if (this.state.selectedVariantId !== 0) {
      variantModels = _.map(this.state.data[this.state.selectedVariantId].Models, (model, i) => {
        variantModelClass = '';
        ...
        if (model.WorkspaceId === 1) {
          ...
          const cultureOptions = resolveModelLanguages({ 
            availableLanguages: this.props.initialData.LanguagesAvailable,
            cultures: this.props.cultures,
            languages: this.props.Languages,
            currentLanguage: model?.LanguageCode,
          ...
          if (this.state.isEditName && model.Id == this.state.selectedModelId) {
          ...
          _.each(this.state.countriesSelected, (country) => {
            ...
            >
              ...
              <td>
                <div className="input-group">
                  <input
                    type="text"
                    id={`v${self.state.selectedVariantId}m${model.Id}t${model.Type.substring(
                      0,
                      2
                    )}-name`}
                  ...
                  />
                  <span className="input-group-btn">{buttonValidateEditName}</span>
                </div>
              </td>
              <td>
                <select
              ...
              <td>
                <select
                  className="model-country form-control"
                  defaultValue={model?.CountryId}
                  data-id={model?.Id}
                  data-type={model?.Type}
                  onChange={self.changeCountry}
                >
                  {countriesOptions}
                </select>
              </td>
              <td>
                <select
                  className="model-language form-control"
              ...
              <td>
                <Dropdown
                  ...
                  onChange={(selectedSubset) => self.setValue(selectedSubset, currentSubsets)}
                  options={onflySubsets}
                  value={currentSubsets}
                  hasOptionTags
                  placeholder={this.props.resources.ContentManagement.SearchSubset}
                  canDeleteItem={false}
                />
              </td>
              <td className="text-right">
                <a className="delete-btn" onClick={() => self.prepareDeleteModel(model)}>
    ...
    const retrieveExtensions = (extensionsType) => this.state.extensions.map((ext) => ext[extensionsType] ? ext.Suffix : null).filter((e)=>e);
    const model3DExtension = retrieveExtensions('Is3D');
    const model2DExtension = retrieveExtensions('Is2D');
    const modelComplementExtension = retrieveExtensions('IsComplement');
    const modelPreviewExtension = retrieveExtensions('IsPreview');
            ...
            <table className="table table-striped" id="bimobject-variants">
              <thead>
                <tr>
                  ...
                  <th>{this.props.resources.EditModelsPage.VariantsDesignation}</th>
                  <th>{this.props.resources.EditModelsPage.VariantsSoftwares}</th>
                ...
                <table className="table table-striped" id="bimobject-models">
                  <thead>
                    <tr>
                      ...
                      <th>{this.props.resources.EditModelsPage.ModelSoftware}</th>
                      <th>{this.props.resources.EditModelsPage.ModelLod}</th>
                      <th>{this.props.resources.EditModelsPage.ModelCountry}</th>
                      <th>{this.props.resources.EditModelsPage.ModelLanguage}</th>
                  ...
                  <div className="row">
                    <div className="col-md-11">
                      <div className={variantPreviewClass}>
                        <label>{this.props.resources.EditModelsPage.PreviewModel}</label>
                        <Dropzone
                          multiple={false}
                        ...
                        >
                          {({ getRootProps, getInputProps }) => (
                            <div {...getRootProps({ className: 'dropzone-area' })}>
                              ...
                              <p className="legende">
                                {`${this.props.resources.EditModelsPage.SupportedFiles} ${modelPreviewExtension.join(', ')}`}
                                <br />
                                {this.props.resources.EditModelsPage.SizeMax.replace('{0}', '30')}
                              </p>
                            </div>
                          )}
                        </Dropzone>
                        <div className={variantPreviewTableClass}>
                          <table className="table table-striped" id="bimobject-models-preview">
                            <thead>
                              <tr>
                                <th>{this.props.resources.EditModelsPage.ModelFileName}</th>
                                <th>{this.props.resources.EditModelsPage.ModelExtension}</th>
                                <th>{this.props.resources.EditModelsPage.ModelSize}</th>
                                <th />
                              </tr>
                            </thead>
                    ...
                    <div className="col-md-11 col-md-offset-1">
                      <div className={variantComplementClass}>
                        <label>{this.props.resources.EditModelsPage.ComplementFile}</label>
                        <Dropzone
                          multiple={false}
                        ...
                        >
                          {({ getRootProps, getInputProps }) => (
                            <div {...getRootProps({ className: 'dropzone-area' })}>
                              ...
                              <h4>{this.props.resources.EditModelsPage.DragBox}</h4>
                              <p className="legende">
                                {`${this.props.resources.EditModelsPage.SupportedFiles} ${modelComplementExtension.join(', ')}`}
                                <br />
                                {this.props.resources.EditModelsPage.SizeMax.replace('{0}', '30')}
                              </p>
                            </div>
                        ...
                        <div className={variantComplementTableClass}>
                          <table className="table table-striped" id="bimobject-models-complement">
                            <thead>
                              <tr>
                                <th>{this.props.resources.EditModelsPage.ModelFileName}</th>
                                <th>{this.props.resources.EditModelsPage.ModelExtension}</th>
        ...
        <div id="gestion-modele" className="panel edit-object">
          ...
          <div className="col-md-11">
            ...
            <Dropzone
               multiple={false}
              onDropRejected={this.onFileRejected}
              onDropAccepted={self.on3dModelDrop}
              maxSize={104857600}
              accept={{
                'model/obj': model3DExtension,
              }}
            >
              {({ getRootProps, getInputProps }) => (
                <div {...getRootProps({ className: 'dropzone-area' })}>
                  ...
                  <h4>{this.props.resources.EditModelsPage.DragBox}</h4>
                  <p className="legende">
                    {`${this.props.resources.EditModelsPage.SupportedFiles  } ${  model3DExtension.join(', ')}` }
                    <br />
                    {this.props.resources.EditModelsPage.SizeMax.replace('{0}', '30')}
                  </p>
                </div>
          ...
          <div className="col-md-11 col-md-offset-1">
            ...
            <Dropzone
              multiple={false}
              onDropRejected={this.onFileRejected}
              onDropAccepted={this.on2dModelDrop}
              accept={{
                '*/*': model2DExtension,
              }}
              maxSize={104857600}
            >
              {({ getRootProps, getInputProps }) => (
                <div {...getRootProps({ className: 'dropzone-area' })}>
                  ...
                  <h4>{this.props.resources.EditModelsPage.DragBox}</h4>
                  <p className="legende">
                    {`${this.props.resources.EditModelsPage.SupportedFiles  } ${  model2DExtension.join(', ')}` }
                    <br />
                    {this.props.resources.EditModelsPage.SizeMax.replace('{0}', '30')}
                  </p>
                </div>
            ...
            <div className="modal-content">
              <div className="modal-header">
                ...
                <h4 className="modal-title" id="myModalLabel">
                  {this.props.resources.EditModelsPage.SelectVariantsModelModalTitle}
                </h4>
              </div>
                ...
                <div>
                  <label>
                    <input
                      ...
                      onClick={self.handleClickOnCheckbox}
                    />
                    <strong>{this.props.resources.DeleteModelModal.ModalAllVariants}</strong>
                  </label>
                  <br />
                  <br />
                </div>
              ...
              <div className="modal-footer">
                ...
                <button type="button" className="btn-second btn-blue" onClick={this.validateUpload}>
          ...
          <div className="modal-dialog">
            <div className="modal-content">
              ...
              <div className="modal-footer">
                ...
                <button type="button" className="btn-second btn-red" onClick={this.deleteModel}>
...
const mapDispatchToProps = (dispatch) => ({
  ...
  updateSubsetTwoDModelReference: (bimObjectId, modelId, variantId, subsetsIds) =>
    dispatch(updateSubsetTwoDModelReferenceAction(bimObjectId, modelId, variantId, subsetsIds)),
  updateSubsetThreeDModelReference: (bimObjectId, modelId, variantId, subsetsIds) =>
    dispatch(updateSubsetThreeDModelReferenceAction(bimObjectId, modelId, variantId, subsetsIds)),
});

export default EditorStepModels = connect(mapStateToProps, mapDispatchToProps)(withCountriesCultures(EditorStepModels));
