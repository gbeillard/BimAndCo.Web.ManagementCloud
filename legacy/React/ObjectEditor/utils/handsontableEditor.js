...
const DATA_LANGUAGE_CODE = 'data-language-code';
const DATA_DOMAIN_ID = 'data-domain-id';
...
export function returnHandsontableColumnAdditionalParameters(
  ...
  const item = new Object();
  if (property.IsTable) item.type = 'text';
  else {
...
export function createHandsontableInstance(
  ...
  const dataLanguageCodeAttribute = `${DATA_LANGUAGE_CODE}='${lang.LanguageCode}'`;
  const dataDomainIdAttribute = `${DATA_DOMAIN_ID}='${domain.Id}'`;

  $(mainContainer).append(
    `<div class='inactive' id='${handsontableId}' ${dataLanguageCodeAttribute} ${dataDomainIdAttribute}></div>`
  );
...
export function findHandsontableInstanceSettings(hot, type) {
  if (type == 'domain_id') {
    return hot.rootElement.dataset?.domainId;
  }
  if (type == 'lang') {
    return hot.rootElement.dataset?.languageCode;
  }
}
