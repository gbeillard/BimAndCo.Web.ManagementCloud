...
import { Button, H1, P, defaultTheme, space } from '@bim-co/componentui-foundation';
...
const Editor = createReactClass({
    ...
    fetch(
      ...
      .then((json) => {
        ...
        store.dispatch({
          ...
          data: `${
            json.Name
              ? `${self.props.resources.EditionPages.MainBimObjectTitle} : ${json.Name}`
              : ''
          }`,
        });
      });
  ...
  setIsTemplate(isTemplate) {
    this.setState((previousState) => ({
      initialData: { ...previousState.initialData, IsTemplate: isTemplate },
    }));
  },
    ...
    if (this.state.statusCode === 401) {
      return (
        <Container>
          ...
          <P>
            {this.props.statusCode} {this.props.resources?.UserAccount.NoAuthorize}
          </P>
        </Container>
      );
    }
    ...
    if (this.props.contextEditor == 'create' && this.state.permissions != null) {
      ...
      contentEditor = (
        <EditorStepOne
          ...
          refreshInformation={this.getBimObjectInformations}
          setIsTemplate={this.setIsTemplate}
        />
      );
      currentInfo = 'current';
      stepButtons = (
