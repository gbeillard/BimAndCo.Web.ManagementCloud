...
export default withCountriesCultures(function PropertiesImportTxt({
  resources,
  ...
  useEffect(() => {
    if(!languageTxtImport){
      changeLanguageTxtImport(cultures?.[0]?.Code)
    }
  }, [cultures]);
          ...
          <DialogContent>
            <SelectWrapper id="mapping-configuration-language-container">
              <FormControl id="mapping-configuration-language">
                ...
                >
                  {cultures.map(({ Name, Code }) => (
                    <MenuItem
                    key={`lang-${Code}`}
                    className="menu-item-language"
                    value={Code}
                  >
                    <ListItemText inset primary={Name} className="lang-name manage-item" />
                  </MenuItem>
                  ))}
                </Select>
              </FormControl>
              ...
              >
                {({ getRootProps, getInputProps }) => (
                  <div {...getRootProps({ className: 'dropzone-upload-file dropzone-area' })}>
                    ...
                    <Typography
                      className="dropzone-subtitle legende"
                      variant="subtitle1"
                      gutterBottom
                    >
