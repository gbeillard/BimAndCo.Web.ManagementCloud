...
import { TablePropertyElement } from './TablePropertyElement';
...
const PropertyElement = createReactClass({
  getInitialState() {
    return {
      ...
      propertyDatas: {
        ...
        PropertyName: '',
        PropertyDescription: '',
        PropertyLang: store.getState().appState.Language,
        PropertyDomain: 0,
        PropertyDomainType: '',
        PropertyUnit: 0,
        PropertyType: 0,
      ...
      enableTableProperties: store.getState().appState.Settings.EnableTableProperties,
    };
  },
  ...
  mapCurrentDatas() {
    ...
    if (dataModel !== undefined && dataModel !== null) {
      ...
      if (currentCopyDatas.Translations !== undefined && currentCopyDatas.Translations !== null) {
        ...
        currentProps.IsTable = currentCopyDatas.IsTable;
        currentProps.SchemaTable = currentCopyDatas.SchemaTable;
      ...
      const hasLanguage = cultures?.some(culture => culture?.Code === language);
      const defaultLanguage = hasLanguage ? language : cultures?.[0]?.Code;

      const emptyDatas = {
        PropertyId: 0,
        PropertyName: '',
        PropertyDescription: '',
        PropertyLang: defaultLanguage,
        PropertyDomain: 0,
        PropertyDomainType: '',
        PropertyUnit: 0,
        PropertyType: 0,
  ...
  handleChangeName(event) {
    ...
    if (this.state.isEditionMode || this.state.isImportBcMode === true) {
      ...
      if (translationToUpdate) {
        ...
        updatedTranslations = [...this.state.currentDataModel.Translations, updatedTranslation];
      }
  ...
  setIsTable(isTable) {
    if (this.state.isImportBcMode != true) {
      if (this.state.isCreationMode) {
        const currentValue = isTable;
        const currentProperty = this.state.propertyDatas;
        currentProperty.IsTable = currentValue;
        this.setState({ propertyDatas: currentProperty });
      }
      if (this.state.isEditionMode) {
        const currentValue = isTable;
        ...
        currentDatasModel.IsTable = currentValue;
        currentProperty.IsTable = currentValue;
        this.setState({
          propertyDatas: currentProperty,
          currentDataModel: currentDatasModel,
  ...
  setSchemaTable(schemaTable) {
    if (this.state.isImportBcMode != true) {
      if (this.state.isCreationMode) {
        const currentValue = schemaTable;
        const currentProperty = this.state.propertyDatas;
        currentProperty.SchemaTable = currentValue;
        this.setState({ propertyDatas: currentProperty });
      }
      if (this.state.isEditionMode) {
        const currentValue = schemaTable;
        ...
        currentDatasModel.SchemaTable = currentValue;
        currentProperty.SchemaTable = currentValue;
        this.setState({
          propertyDatas: currentProperty,
          currentDataModel: currentDatasModel,
    ...
    if (this.state.isImportBcMode !== true) {
      ...
      if (
        ...
        _.each(currentsLangs, (lang, i) => {
          if (continueToRead) {
            let editTypeValueLang = lang?.PropertyEditTypeValues
        ...
        const editTypeValue = this.state.propertyDatas.PropertyEditTypeValues
          ? this.state.propertyDatas.PropertyEditTypeValues
          : '';
        const splitEditTypeValue = (editTypeValue) =>
          editTypeValue.split(';').filter((string) => string !== '');
        const editTypeValueArray = editTypeValue ? splitEditTypeValue(editTypeValue) : '';
        const isEnoughtTypeValue =
          editTypeValueArray.length < 2 && this.state.propertyDatas.PropertyEditType !== 3;
  ...
  validateAction() {
    ...
    // check schemaTable
    if (
      this.state.propertyDatas.IsTable &&
      (!('Columns' in this.state.propertyDatas.SchemaTable) ||
        this.state.propertyDatas.SchemaTable.Columns.filter((elt) => elt === null).length > 0)
    ) {
      this.setState({ errorFieldSchemaTable: true });
      isOk = false;
    } else {
      this.setState({ errorFieldSchemaTable: false });
    }
  ...
  render() {
    ...
    const hasLanguage = self.props.cultures?.some(culture => culture?.Code === self.state.propertyDatas.PropertyLang);

    if(!hasLanguage){
      const propertyLanguage = this.props.Languages?.find(language => language?.LanguageCode === self.state.propertyDatas.PropertyLang);

      let languageTrad = propertyLanguage?.DefaultName;

      if (propertyLanguage?.Translations?.[self.props.Language]) {
        languageTrad = propertyLanguage?.Translations[self.props.Language];
      }

      languageList.unshift(
        <MenuItem key={`lang-${propertyLanguage?.LanguageCode}`} value={propertyLanguage?.LanguageCode}>
          {languageTrad}
        </MenuItem>
      );
    }
      ...
      <>
        ...
        <DialogContent>
          <div>
            ...
            {this.state.enableTableProperties && (
              <TablePropertyElement
                isTable={self.state.propertyDatas.IsTable}
                setIsTable={self.setIsTable}
                schemaTable={
                  self.state.propertyDatas.SchemaTable ? self.state.propertyDatas.SchemaTable : null
                }
                setSchemaTable={self.setSchemaTable}
                resources={self.props.Resources}
                errorFieldSchemaTable={self.state.errorFieldSchemaTable}
              />
            )}
...
export default withCountriesCultures(PropertyElement);
