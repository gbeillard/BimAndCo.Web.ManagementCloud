...
import { Icon, Input } from '@bim-co/componentui-foundation';
...
let DictionaryProperties = createReactClass({
    ...
    fetch(
      `${API_URL}/api/ws/v1/${self.props.Language}/contentmanagement/${self.props.ManagementCloudId}/dictionary/property/create?token=${self.props.TemporaryToken}`,
      {
        ...
        body: JSON.stringify({
          ...
          IsTable: currentElements.IsTable,
          SchemaTable: self.mapSchemaTableToBack(currentElements.SchemaTable),
        }),
      }
    ).then((response) => {
  ...
  loadPropertyDetails(propertyId) {
    ...
    fetch(
      ...
      .then((json) => {
        if (isResponseOk) {
          json.SchemaTable = self.mapSchemaTableToFront(json.SchemaTable);
          self.setState({
  ...
  updateProperty(propertyElements) {
    ...
    const self = this;
    currentData.SchemaTable = self.mapSchemaTableToBack(currentData.SchemaTable);
  ...
  uploadRevitParameterFile(file) {
    ...
    if (file != null) {
      ...
      fetch(
        `${API_URL}/api/ws/v1/${self.props.Language}/contentmanagement/${self.props.ManagementCloudId}/dictionary/properties/revitupload/${self.state.languageTxtImport}/read?token=${self.props.TemporaryToken}`,
        {
          method: 'POST',
          body: data,
        }
  ...
  uploadDataImport() {
    ...
    const currentDatas = JSON.stringify({
      ...
      LanguageCodeImport: self.state.languageTxtImport,
    });
  ...
  mapSchemaTableToFront(schemaTableBack) {
    if (schemaTableBack && 'Columns' in schemaTableBack && schemaTableBack.Columns.length > 0)
      return {
        ...schemaTableBack,
        Columns: schemaTableBack.Columns.map((elt) =>
          this.props.allCurrentProperties.find((property) => elt.Id === property.Id)
        ),
      };
    return null;
  },

  mapSchemaTableToBack(schemaTableFront) {
    if (schemaTableFront && 'Columns' in schemaTableFront && schemaTableFront.Columns.length > 0)
      return {
        ...schemaTableFront,
        Columns: schemaTableFront.Columns.map((elt) => ({
          Id: elt.Id,
          Guid: 'Guid' in elt ? elt.Guid : '',
          EntityType: 0,
          Value: '',
        })),
      };
  ...
  showDeleteIcon(event) {
    ...
    const currentIcont = document.getElementById(`property-translation-${currentLine}`);
    currentIcont.style.display = 'block';
  },
  ...
  hideDeleteIcon(event) {
    ...
    const currentIcont = document.getElementById(`property-translation-${currentLine}`);
    currentIcont.style.display = 'none';
  },
  ...
  mapImportDatasEdit(id) {
    ...
    let currentLineDetailsLangIndex = currentLineDetails.PropertyLangs.findIndex(
      (lang) => lang.LangCode.toLowerCase() === self.props.Language.toLowerCase()
    );

    currentLineDetailsLangIndex = currentLineDetailsLangIndex >=0 ? currentLineDetailsLangIndex : 0;
        ...
        >
          ...
          <TableCell className="domain-table-column-body text-right" id={prop.Id}>
            <div style={{ display: 'flex', flexDirection: 'row' }}>
              <div
                className="property-translation domain-table-column-body text-right"
                id={`property-translation-${prop.Id}`}
                style={{ display: 'none', marginRight: '10px' }}
                onClick={() =>
                  history.push(`/${self.props.Language}/dictionary/properties/${prop.Guid}`)
                }
              >
                <Icon size="l" icon="traduction" />
              </div>
              <DeleteIcon
      ...
      <div>
        ...
        <div id="underBandSelection" className={underBandClass}>
          <div className="underBandSelection-padding">
            <span>
              {`${self.props.allCurrentProperties.filter((p) => p.selected === true).length} ${
                self.props.Resources.ContentManagement.PropertiesSelected
              }`}
            </span>
