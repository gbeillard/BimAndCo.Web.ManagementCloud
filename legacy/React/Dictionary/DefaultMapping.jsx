...
let DefaultMapping = createReactClass({
  ...
  handleDefaultSearch(searchText) {
    ...
    const result = [
      ...this.state.FullPropertiesDomainList.map((domain, i) => {
        const fuse = new Fuse(domain.PropertyList, { ...options });
        this.state.FullPropertiesDomainList[i].PropertyList = fuse
          .search(searchText)
          .map(({ item }) => item);
        return domain;
      }),
  ...
  handleSearchChange(event) {
    ...
    setTimeout(() => {
      if (self.state.SearchText === searchText && searchText !== '') {
        this.setState({
          PropertiesDomainList: renderSearchResult || [],
        });
      }
    }, 1400);
                ...
                <FormControl id="mapping-configuration-language">
                  ...
                  >
                    ...
                    {cultures.map((lang) => (
                        ...
                        value={lang.Code}
                      >
                        <ListItemText inset primary={lang.Name} className="lang-name" />
                      </MenuItem>
                    ))}
                  </Select>
                </FormControl>
...
export default withDecorators(DefaultMapping);
