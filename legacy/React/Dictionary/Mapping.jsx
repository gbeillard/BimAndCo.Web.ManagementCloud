...
import { H3, Icon } from '@bim-co/componentui-foundation';
...
import { getMappingCultures } from './utils';
...
let Mapping = createReactClass({
  ...
  render() {
    ...
    const cultures = getMappingCultures({ 
      ...
      <div id="dropzone-container">
        ...
        >
          {({ getRootProps, getInputProps }) => (
            <div {...getRootProps({ className: 'dropzone-upload-file dropzone-area' })}>
              <input {...getInputProps()} />
    ...
    switch (this.state.PageMode) {
      ...
      case mode.NAMING_CONVENTION:
        if (this.state.ConfigurationExist) {
          let configurationList = null;
          if (this.props.isDefault) {
            configurationList = _.map(this.state.MappingDictionaryList, (configuration, i) => {
              if (configuration.IsDefault) {
                this.handleSelectConfiguration(configuration.Id);
                return <H3>{configuration.Name}</H3>;
              }
            });
          } else {
            configurationList = _.map(this.state.MappingDictionaryList, (configuration, i) => {
              ...
              return (
                <MenuItem
                  ...
                  className="menu-item-configuration"
                  value={configuration.Id}
                >
                  <ListItemText inset primary={configuration.Name} className="configuration-name" />
                  {configuration.IsPluginDefault && <Icon color="primary" icon="check" size="l" />}
                </MenuItem>
              );
            });
          }
            ...
            mappingLanguage = (
              <div id="mapping-configuration-language-container">
                <FormControl id="mapping-configuration-language">
                  ...
                  >
                    {cultures.map(({ Code, Name }) => (
                       <MenuItem
                       key={`lang-${Code}`}
                       className="menu-item-language"
                       value={Code}
                     >
                       <ListItemText inset primary={Name} className="lang-name" />
                     </MenuItem>
                    ))}
                  </Select>
                </FormControl>
        ...
        if (this.state.Id > 0) {
          ...
          mappingLanguage = <span>{`(${this.state.Language})`}</span>;
        }
...
const withDecorators = compose(
  ...
  withRouter,
  withCountriesCultures,
);

export default withDecorators(Mapping);
