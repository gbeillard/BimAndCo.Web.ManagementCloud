...
import { CountriesCulturesProvider, OCAnalytics } from '@bim-co/onfly-connect';
...
const Onfly = (props) => {
  ...
  const appConfig = useMemo(
    () => ({ apiUrl: API_URL, resources, token: TemporaryToken, language, onflyId: ManagementCloudId }),
    [ManagementCloudId, language, resources, TemporaryToken]
  );
  ...
  useUpdateEffect(() => {
    if (isPlugin) {
      ...
      OCAnalytics.config({ source: 'plugin', pluginVersion, software, softwareVersion });
    }
  }, [isPlugin]);
  ...
  return (
    <CountriesCulturesProvider appConfig={appConfig}>
